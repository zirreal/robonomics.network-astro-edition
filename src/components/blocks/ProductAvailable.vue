<template>
  <section class="product-available">
    <section class="lined">
      ...
      <PriceSection :price="price" :message1="$tr('Now Available!')" :message2="$tr('Worldwide Shipping')" />
      <ul>
        <li class="italic-text" v-for="item in info" :key="item">{{$tr(item)}}</li>
      </ul>
    </section>
    <div class="product-buy__container" :class="{ 'active oldy': showOrder }">
      <robo-button class="block" @click="$emit('toggle-order')">
        {{$tr('Order Now')}}
      </robo-button>
...
const props = defineProps({
  title: String, price: String, showOrder: Boolean, orderLinks: Array, info: Array
})
</script>
